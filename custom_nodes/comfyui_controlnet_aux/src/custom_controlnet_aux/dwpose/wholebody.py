    ...
    def __init__(self, det_model_path: Optional[str] = None, pose_model_path: Optional[str] = None, torchscript_device="cuda"):
        ...
        if self.det_model_type is None:
            ...
            try:
                ...
                sess_options.inter_op_num_threads = int(os.environ["OMP_NUM_THREADS"])
                sess_options.intra_op_num_threads = int(os.environ["OMP_NUM_THREADS"])
                sess_options.log_severity_level = int(os.environ["ORT_LOG_LEVEL"])
                self.det = ort.InferenceSession(det_model_path, sess_options, providers=ort_providers)
            except:
                ...
                self.det = ort.InferenceSession(det_model_path, sess_options, providers=["CPUExecutionProvider"])
        ...
        if self.pose_model_type is None:
            ...
            try:
                ...
                sess_options.intra_op_num_threads = int(os.environ["OMP_NUM_THREADS"])
                sess_options.log_severity_level = int(os.environ["ORT_LOG_LEVEL"])
                self.pose = ort.InferenceSession(pose_model_path, sess_options, providers=ort_providers)
            except:
                print(f"Failed to load onnxruntime with {self.pose.get_providers()}.\nPlease change EP_list in the config.yaml and restart ComfyUI")
                self.pose = ort.InferenceSession(pose_model_path, sess_options, providers=["CPUExecutionProvider"])
